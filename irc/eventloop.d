...
class IrcEventLoop
{
    ...
    this()
    {
        ...
        asyncMutex = new Mutex();
    }

    ~this()
    {
        // Stop and clean up async watcher
        if(ev_is_active(&asyncWatcher))
            ev_async_stop(ev, &asyncWatcher);
    ...
    {
        enforce!UnconnectedClientException(
    ...
    private extern(C) static void onCustomTimeout(ev_loop_t* ev, ev_timer* timer, int revents)
    {
        ...
        // If it's a one-shot timer, clean it up after execution
        if(customTimer.timer.repeat == 0)
        {
            GC.removeRoot(customTimer);
            // If we were using manual allocation, we'd free here:
            // dealloc(customTimer);
        }
     ...
     * This is useful when integrating with other event loops or GUI frameworks.
     */
    void runOnce()
    {
        ev_run(ev, EVRUN_ONCE);
    }

    /**
     * Run the event loop without blocking.
     * Processes pending events without waiting for new ones.
     * This is the most non-blocking option available.
     */
    void runNowait()
    {
        ev_run(ev, EVRUN_NOWAIT);
    }

    /**
     * Break out of the event loop.
     * This can be called from another thread to stop the event loop.
     * Params:
     *   how = how many loops to break out of (use EVBREAK_ONE for this loop only)
     */
    void breakLoop(int how = EVBREAK_ONE) @trusted
    {
        ev_break(ev, how);
    }

    /**
     * Set an async callback that can be triggered from another thread.
     * When sendAsync() is called from any thread, this callback
     * will run in the event loop thread.
     *
     * Params:
     *   callback = The function to call when async event is triggered
     */
    void setAsyncCallback(void delegate() callback)
    {
        asyncMutex.lock();
        scope(exit) asyncMutex.unlock();

        asyncCallback = callback;

        if (callback && !ev_is_active(&asyncWatcher))
        {
            ev_async_init(&asyncWatcher, &onAsync);
            asyncWatcher.data = cast(void*)this;
            ev_async_start(ev, &asyncWatcher);
        }
        else if (!callback && ev_is_active(&asyncWatcher))
        {
            ev_async_stop(ev, &asyncWatcher);
        }
    }
    ...
    /**
     * Send async signal from another thread to wake up event loop.
     * Safe to call from any thread.
     *
     * This will trigger the async callback set via setAsyncCallback()
     * to run in the event loop thread.
     */
    void sendAsync() @trusted
    {
        asyncMutex.lock();
        scope(exit) asyncMutex.unlock();

        if (ev_is_active(&asyncWatcher) && asyncCallback)
        {
            ev_async_send(ev, &asyncWatcher);
        }
    }

    /**
     * Check if there's a pending async event.
     * Returns true if an async event has been sent but not yet processed.
     */
    bool asyncPending() @property
    {
        asyncMutex.lock();
        scope(exit) asyncMutex.unlock();

        return ev_is_active(&asyncWatcher) && ev_async_pending(&asyncWatcher);
    }

    /**
     * Get the underlying libev loop for advanced integration.
     * Use with caution - direct manipulation can break the event loop.
     */
    ev_loop_t* getEvLoop() @property
    {
        return ev;
    }
    ...
    {
        // libev doesn't have a direct "is running" API, but we can check
        // if we have any active watchers as a proxy
        if (!watchers.empty) return true;
        if (dccWatchers !is null) return true;
        if (ev_is_active(&idleWatcher)) return true;
        if (ev_is_active(&asyncWatcher)) return true;

        return false;
    }

    /**
     * Process pending events without running the main loop.
     * Useful for integrating with other event systems.
     */
    void processPending()
    {
        ev_invoke_pending(ev);
    }

    /**
     * Update the event loop's time cache.
     * Useful for long-running operations outside the event loop.
     */
    void updateNow()
    {
        ev_now_update(ev);
     ...
     * Returns the cached time from the last event loop iteration.
     */
    double now() @property
    {
        return ev_now(ev);
    ...
    /**
     * Get the backend used by this event loop.
     * Returns a bitmask of EVBACKEND_* constants.
     */
    uint backend() @property
    {
        return ev_backend(ev);
    }

    /**
     * Suspend the event loop temporarily.
     * Useful for debugging or when integrating with other systems.
     * Must be paired with a call to resume().
     */
    void suspend()
    {
        ev_suspend(ev);
    ...
    /**
     * Resume a suspended event loop.
     * Must be called after suspend().
     */
    void resume()
    {
        ev_resume(ev);
     ...
     */
    uint pendingCount() @property
    {
        return ev_pending_count(ev);
    }

    private:
    private extern(C) static void onAsync(ev_loop_t* ev, ev_async* watcher, int revents)
    {
        auto eventLoop = cast(IrcEventLoop)watcher.data;

        eventLoop.asyncMutex.lock();
        scope(exit) eventLoop.asyncMutex.unlock();

        if (eventLoop.asyncCallback)
        {
            try
            {
                eventLoop.asyncCallback();
            }
            catch (Exception e)
            {
                // Log the error but don't crash the event loop
                import std.stdio : stderr;
                stderr.writefln("Error in async callback: %s", e.msg);
            }
        }
    }
}
